  ...
  pipeline {
    ...
    stages {
      ...
      stage('Run & Smoke Test') {
        steps {
          script {
            // Run container and wait for app to respond. If it doesn't, fail the build and print logs.
            sh '''
              set -e
      ...
      stage('Archive') {
        steps {
          archiveArtifacts artifacts: 'Dockerfile,run.py,requirements.txt', fingerprint: true
        }
      }
    }
